    ...
    `;this.elementSearch()},this.elementSearch=()=>{this.chatInput=document.getElementById("chat-input");this.chatSubmit=document.getElementById("chat-submit");this.chatMessages=document.getElementById("chat-messages");this.chatBubble=document.getElementById("chat-bubble");this.chatPopup=document.getElementById("chat-popup");this.closePopup=document.getElementById("close-popup");this.chatHistory=document.getElementById("chat-history")},this.autoSizeWidth=()=>{if(window.innerWidth<768){return}if(this.chatMessages.clientHeight<this.chatMessages.scrollHeight){this.chatMessages.style.paddingRight="13px"}},this.messagesContainerCheckWidth=()=>{if(window.innerWidth>768&&this.chatMessages.clientHeight<this.chatMessages.scrollHeight){this.chatMessages.style.paddingRight="13px"}else{this.chatMessages.style.paddingRight="16px"}},this.resizeChatWidth=()=>{window.addEventListener("resize",()=>{this.messagesContainerCheckWidth()})},this.textareaCheckWidth=()=>{if(window.innerWidth<768){return}const textareaMaxHeight=parseFloat(getComputedStyle(this.chatInput).maxHeight);if(textareaMaxHeight<this.chatInput.scrollHeight){this.chatInput.style.paddingRight="13px"}else{this.chatInput.style.paddingRight="16px"}},this.textareaAutoSizeWidth=()=>{this.chatInput.addEventListener("input",()=>{this.textareaCheckWidth()})},this.textareaCheckHeight=()=>{this.chatInput.style.height="36px";this.chatInput.style.height=this.chatInput.scrollHeight+"px"},this.textareaAutoHeight=()=>{this.chatInput.addEventListener("input",()=>{this.textareaCheckHeight()})},this.sendingToChat=()=>{this.chatSubmit.addEventListener("click",async()=>{const message=this.chatInput.value.trim();if(!message)return;this.chatMessages.scrollTop=this.chatMessages.scrollHeight;this.onUserRequest(message);globalThis.prints.printsAdd();this.autoSizeWidth();this.textareaCheckHeight()})},this.sendingUsingEnter=()=>{this.chatInput.addEventListener("keyup",event=>{if(event.key==="Enter"){this.chatSubmit.click();this.autoSizeWidth()}})},this.textareaCancelNewlineOn=()=>{this.chatInput.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault()}})},this.openChat=()=>{this.chatBubble.addEventListener("click",()=>{this.togglePopup()})},this.closeChat=()=>{this.closePopup.addEventListener("click",()=>{this.togglePopup()})},this.togglePopup=()=>{this.chatPopup.classList.toggle("hidden")},this.scrollWindowToBottom=()=>{this.chatPopup.classList.remove("hidden");this.chatMessages.scrollTop=this.chatMessages.scrollHeight;this.chatPopup.classList.add("hidden")},this.chatAutoStart={init(){this.setTimeoutAutoStart()},setTimeoutAutoStart(){if(globalThis.config?.seconds_to_autostart){setTimeout(()=>{if(globalThis.checkFirstOpening.isOpened)return;if(globalThis.chatPopup.classList.contains("hidden")){globalThis.chatBubble.click()}},globalThis.config?.seconds_to_autostart*1e3)}}};this.userMessageAppend=message=>{const messageElement=this.getUserMessage(message);this.chatMessages.appendChild(messageElement);this.chatMessages.scrollTop=this.chatMessages.scrollHeight;this.chatInput.value=""},this.onUserRequest=message=>{try{this.userMessageAppend(message);this.ws.send(JSON.stringify({action:"MESSAGE",session_id:localStorage.getItem("session_id"),message:message}))}catch(e){console.log(e)}},this.getBotMessage=message=>{const parseMessage=globalThis.convertingFunction.parseLinks(message);const bMessage=document.createElement("div");bMessage.className="flex mb-3 chat-message-bot-container";bMessage.innerHTML=`<div class="chat-message-bot">${parseMessage}</div>`;return bMessage},this.getUserMessage=message=>{const uMessage=document.createElement("div");uMessage.className="chat-message-user-container";uMessage.innerHTML=`<div class="chat-message-user">${message}</div>`;return uMessage},this.loadMessageHistory=()=>{this.config.conversation_history.forEach(({author,message})=>{this.chatHistory.append(author==="MAIA"||author==="AI_AGENT"?this.getBotMessage(message):this.getUserMessage(message))});this.scrollWindowToBottom()},this.reply=message=>{this.messageSound.audioPlay();const replyElement=this.getBotMessage(message);this.chatMessages.appendChild(replyElement);this.chatMessages.scrollTop=this.chatMessages.scrollHeight;this.autoSizeWidth()},this.openWebSoket=(domain="api.dev.maia.work")=>{const{appId,apiHash}=document.querySelector("#maia-chat-widget").dataset;this.ws=new WebSocket(`wss://${domain}/api/v1/webhook/chat_widget?app_id=${appId}&api_hash=${apiHash}`);this.webSocketOpenHandler();this.webSocketMessageHandler();this.webSocketCloseHandler();this.websocketClosure()},this.webSocketOpenHandler=async()=>{const helpers=globalThis.helpers;const utm=helpers.getUTMData();const locationData=await helpers.getLocationData();this.ws.onopen=e=>{this.ws.send(JSON.stringify({action:"INIT",session_id:localStorage.getItem("session_id")?localStorage.getItem("session_id"):undefined,metadata:{utm_data:{campaign:utm?.["utm_campaign"]?utm?.["utm_campaign"]:"",source:utm?.["utm_source"]?utm?.["utm_source"]:"",medium:utm?.["utm_medium"]?utm?.["utm_medium"]:"",search_term:utm?.["utm_search_term"]?utm?.["utm_search_term"]:""},referring_url:helpers.getReferrerURL(),timestamp:helpers.getTimeStamp(),location_data:locationData}}))}},this.webSocketMessageHandler=()=>{this.ws.onmessage=e=>{const response=JSON.parse(JSON.parse(e.data));if(response.action==="INIT"){if(localStorage.getItem("session_id")){}else{localStorage.setItem("session_id",response.session_id)}this.writingDataToConfig(response);if(!this.checkDomen())return false;this.init();this.messageSound.init();this.iconForUnreadMessage.init()}if(response.action==="MESSAGE"){this.reply(response.answer);this.iconForUnreadMessage.messageCheck();globalThis.prints.printsRemove()}}},this.webSocketCloseHandler=()=>{this.ws.onclose=e=>{this?.chatInput?.setAttribute("disabled",true)}},this.websocketClosure=()=>{document.addEventListener("unload",()=>{this.ws.close()})};this.writingDataToConfig=rspns=>{this.config={...this.config,salesperson_name:rspns.salesperson_name,salesperson_role:rspns.salesperson_role,chat_title:rspns.chat_title,circle_color:rspns.circle_color,header_color:rspns.header_color,seconds_to_autostart:rspns.seconds_to_autostart,send_button_color:rspns.send_button_color,user_message_color:rspns.user_message_color,widget_location:rspns.widget_location,conversation_history:rspns.conversation_history,allowed_domains:rspns.allowed_domains}},this.checkDomen=()=>{const currentHref=window.location.href;if(currentHref.includes(this.config.allowed_domains[0])){return true}else{console.log("Ваш домен не включен в список доступных доменов");return false}};this.prints={callArray:[],printsLoadingChoiceActionStart(){this.callArray.push(true);if(this.callArray.length===1){this.printsAdd()}},printsLoadingChoiceActionFinish(){if(this.callArray.length===1){this.printsRemove()}this.callArray.pop()},printsAdd(){const printsContainer=document.querySelector(".chat-status");if(printsContainer.querySelector("[data-chat-prints-loading]"))return;printsContainer.prepend(this.getPrintsHTML())},printsRemove(){const printsContainer=document.querySelector(".chat-status [data-chat-prints-loading]");printsContainer?.remove()},getPrintsHTML(){const prints=document.createElement("div");prints.innerHTML=`<div class="chat-prints" data-chat-prints-loading>
                                      <div class="chat-prints-circle-container">
                                        <div class="chat-prints-circle chat-prints-circle-first"></div>
                                          ...
                                          печатает
                                      </div>`;return prints.firstElementChild}};this.messageSound={init(){this.createAudio()},createAudio(){this.sound=document.createElement("audio");this.sound.src=`https://cabinet.maia.work/static/audio/message_sound.mp3`;this.sound.addEventListener("error",()=>{this.sound=null})},audioPlay(){if(!this.sound)return;if(!this.sound.paused){this.sound.pause();this.sound.currentTime=0}this.sound.play()}};this.iconForUnreadMessage={init(){this.chatOpenHandler()},messageCheck(){if(!this.closeCheck())return;this.showIcon();this.setNotifLocaleStorage()},closeCheck(){const chat=document.querySelector("#chat-popup");if(!chat)return;return chat.classList.contains("hidden")},showIcon(){const icon=document.querySelector("#chat-unread-message-icon");if(!icon)return;icon.classList.remove("hidden")},setNotifLocaleStorage(){localStorage.setItem("unred-message",true)},removeNotifLocaleStorage(){localStorage.removeItem("unred-message")},checkNotifLocaleStorage(){if(localStorage.getItem("unred-message")){this.showIcon()}},hiddenIcon(){const icon=document.querySelector("#chat-unread-message-icon");if(!icon)return;icon.classList.add("hidden")},chatOpenHandler(){const btnOpen=document.querySelector("#chat-bubble");if(!btnOpen)return;btnOpen.addEventListener("click",()=>{if(!this.closeCheck()){this.hiddenIcon();this.removeNotifLocaleStorage()}})}};this.convertingFunction={parseLinks(message){var urlRegex=urlRegex=/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gi;return message.replace(urlRegex,function(url){return'<a href="'+url+'" target="_blank">'+url+"</a>"})}};this.helpers={getTimeStamp(){return Date.now()},async getLocationData(){try{const response=await fetch("https://ipwho.is/");const data=await response.json();return data?.city||""}catch(e){console.error(e);return""}},getUTMData(){const url=window.location.href;const searchParams=Object.fromEntries(new URL(url).searchParams.entries());Object.keys(searchParams).forEach(paramKey=>{if(!paramKey.startsWith("utm_"))delete searchParams[paramKey]});return searchParams},getReferrerURL(){return document.referrer}}}(function(){const chat=new chatWidget;chat.openWebSoket("api.maia.work")})();